...
const EmerdBox=()=>{
  ...
  const handlePredictionColor = async(value:string) => {
    if(Number(Available_Balance) < Number(amount)){
      ...
      Alert.alert("Pleaser enter amount!!!");
        ...
        try{
          const response = await axios.post("https://www.backend.colour-cash.com/api/v1/betting/color",{
              amount: Number(amount),
              period: String(period1),
  ...
  const handlePredictionNumber = async(value:string) => {
    ...
    if(Number(Available_Balance) < Number(amount)){
      ...
      // console.log(token)
        try{
          const response = await axios.post("https://www.backend.colour-cash.com/api/v1/betting/number",{
              amount: Number(amount),
              period: String(period1),
              number: String(value),
              type: 'emerd'
              ...
              {timer1 > 10 ? (
                gamestatus ? (
                  <View style={styles.modalContent}>
                    ...
                    />
                    <TouchableOpacity activeOpacity={1} onPress={isColor(modalContent) ? ()=>handlePredictionColor(modalContent) : ()=>handlePredictionNumber(modalContent)}>
