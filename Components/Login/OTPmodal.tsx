...
const OTPmodal: React.FC<OTPmodalProps> = ({ modalvisible, closemodal, email, openmodal }) => {
  ...
  if (atIndex >= 0) {
    const domain = email.substring(atIndex); // Domain part after '@'
    const username = email.substring(0, atIndex); // Username part before '@'
  ...
  const {token,setforgotPassToken} = useAuth();
  ...
  const handleOTP = async() => {
    if (otpValues.length === 6) {
      try{
        const response = await axios.post("https://www.backend.colour-cash.com/api/v1/auth/reset-password",{
        ...
        <TouchableWithoutFeedback onPress={closemodal}>
          <View style={styles.modalContainer}>
            <TouchableWithoutFeedback onPress={() => {}}>
              ...
              <Text style={styles.header}>An OTP has been sent to your email id {maskedEmail}</Text>
